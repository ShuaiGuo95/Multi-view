...
if Elm_Type~=REGRESSION
    ...
    for i = 1:NumberofTrainingData
        for j = 1:number_class
            if label(1,j) == T(1,i)%指定第j行为1，生成T以后，第j类的话，就是T中第j行对应的训练样本数为1
                break; 
            end
        end
        temp_T(j,i)=1;
...
InputWeight=rand(NumberofHiddenNeurons,NumberofInputNeurons)*2-1; %变为[-1,1]之间的数
...
%%
% faster method 1
 C=800;%2^5;%正则化参数也很重要%2月3日二师兄设置的值为800
